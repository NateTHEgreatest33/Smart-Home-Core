...
def LoraSendMessage( messageList, messageSize):
    ...
    #sleep for 4ms to allow for tx to complete. I think there is an issue in the SPI driver here.
    time.sleep(.04)  #wait for tx co complete

    #read from status register to see if TxDone bit is set (0x08)
    msg = [ 0x12, 0x00 ]
    result = spi.xfer2(msg)

    if( (result[1] & 0x08) != 0x08 ):
        print( "TxDone is not set!" )
...
msgs = [0x01, 0x00, 0x00,    0x14, 0x00, 0xAA, 0xBB, 0xCC, 0xDD ]
msgs.append( updateCRC( msgs ) )
LoraSendMessage(msgs, len(msgs) )
